...
const url = "http://localhost:5058/get-fakeshop"
export const GET_FAKESHOP = createAsyncThunk(
   "get-fakeshop/GET_FAKESHOP", async(_, {rejectWithValue,dispatch})=>{
      try {
...
export const GET_DETAILS_SHOP = createAsyncThunk(
   "get-fakeshop/GET_DETAILS_SHOP", async(id, {rejectWithValue,dispatch})=>{
      try {
         const response = await fetch(`http://localhost:5058/fakeshop/detail/${id}` ,{
            method: "GET",
            headers: {
               "Content-Type": "application/json",
             },
...
export const DELETE_PRODUCT = createAsyncThunk(
   ...
   async (id, { rejectWithValue, dispatch, getState }) => {
      console.log(id);
     try {
       const response = await fetch(`http://localhost:5058/fakeshop/${id}`, {
         method: "DELETE",
       });
       console.log(response);
 ...
 export const EDIT_PRODUCT = createAsyncThunk(
   ...
   async ({_id ,formData}  ,{ rejectWithValue, dispatch, getState }) => {
     ...
     try {
       const response = await fetch(`http://localhost:5058/edit/${_id}`, {
         method: "PATCH",
         headers: {
           "Content-Type": "application/json",
         },
